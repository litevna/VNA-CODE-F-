...
def load_and_process_data(df, num_sets):
    ...
    # Find initial peak for reference
    s11_peaks, _ = find_peaks(np.abs(s11_time), prominence=0.5)
    s21_peaks, _ = find_peaks(np.abs(s21_time), prominence=0.5)

    if len(s11_peaks) > 0:
        peak_time_s11 = time_vector[s11_peaks[0]]
        distance_s11 = peak_time_s11 * 9.891e7 / 2  # Speed of signal in meters per second divided by 2 for round-trip
    else:
        peak_time_s11 = None
        distance_s11 = None
    ...
    if len(s21_peaks) > 0:
        ...
        distance_s21 = peak_time_s21 * 9.891e7 / 2  # Speed of signal in meters per second divided by 2 for round-trip
    else:
        peak_time_s21 = None
        distance_s21 = None
...
def main():
    ...
    if folder_path:
        ...
        output_path = folder_path  # Modify this if you want a different output path
        output_filename = "combined_data"

        # Combine and process data
        df = combine_files_in_folder(folder_path, output_path, output_filename)
        if df is not None:
            uniform_freq, voltage_magnitude_s11, voltage_magnitude_s21, time_axis, peak_time_s11, distance_s11, peak_time_s21, distance_s21 = load_and_process_data(df.copy(), 1)  # Assuming 1 set of data
